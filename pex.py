...
pex_footer_update(pex_c[u"pex_status"], pex_c[u"config_status"],
...
class Settings(ProtectedPage):
    ...
    def GET(self):
        pex_footer_update(pex_c[u"pex_status"], pex_c[u"config_status"],
                          pex_c[u"auto_configure"], pex.pex_msg)
        try:
            sp = template_render.pex(pex_c, pex.edit_conf, gv)
        except Exception as e:
...
class ConfigSave(ProtectedPage):
    ...
    def GET(self):
        ...
        if update_needed:
            ...
            self.pex_c = copy.deepcopy(pex_e)
        pex_footer_update(pex_c[u"pex_status"], pex_c[u"config_status"],
                          pex_c[u"auto_configure"], pex.pex_msg)
        try:
            sp = template_render.pex(pex_c, pex.edit_conf, gv)
...
class scan(ProtectedPage):
    ...
    def GET(self):
        pex_c[u"devices_discovered"] = pex.scan_for_ioextenders(pex_c)
        try:
            sp = template_render.pex(pex_c, pex.edit_conf, gv)
            return sp
        except Exception as e:
